...
static int example_2_handler(request_rec *r);

/* Define our module as an entity and assign a function for registering hooks  */

module AP_MODULE_DECLARE_DATA   example_2_module =
{
...
static void register_hooks(apr_pool_t *pool) 
{
    ...
    ap_hook_handler(example_2_handler, NULL, NULL, APR_HOOK_LAST);
}
...
static int example_2_handler(request_rec *r)
{
    ...
    // Check that the "example2-handler" handler is being called.
    if (!r->handler || strcmp(r->handler, "example2-handler")) return (DECLINED);
    ...
    // If apr_stat failed, we're probably not allowed to check this file.
         ...
         apr_strerror(rc, buffer, BUFFER_SIZE);
         ap_log_error(APLOG_MARK, APLOG_ERR, 0, r->server,
                "Error accessing %s: %s", filename, buffer);
    ...
    ap_rprintf(r, "<b>Size:</b> %lu bytes<br/>", finfo.size);
